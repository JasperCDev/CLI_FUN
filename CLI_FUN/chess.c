...
bool isMoveValidKnight(ChessGame *chessGame, Tile *startTile, Tile *endTile)
{
...
bool isMoveValidPawn(ChessGame *chessGame, Tile *startTile, Tile *endTile)
{
...
bool isMoveValidRook(ChessGame *chessGame, Tile *startTile, Tile *endTile)
{
   bool isRowMove = endTile->rowIndex == startTile->rowIndex;
   bool isUp = endTile->rowIndex < startTile->rowIndex;
   bool isLeft = endTile->colIndex < startTile->colIndex;
   int step = (isUp || isLeft) ? -1 : 1;
   int startIndex = isRowMove ? startTile->colIndex : startTile->rowIndex;
   int endIndex = isRowMove ? endTile->colIndex : endTile->rowIndex;
   int i = startIndex + step;
   while (i != endIndex)
   {
      Tile currTile = isRowMove ? chessGame->board[startTile->rowIndex][i]
...
bool isMoveValidBishop(ChessGame *chessGame, Tile *startTile, Tile *endTile)
{
   bool isAbove = endTile->rowIndex < startTile->rowIndex;
   bool isLeft = endTile->colIndex < startTile->colIndex;
   for (int i = 1; i < 16; i++)
   {
      int rowIndex = isAbove ? startTile->rowIndex - (1 * i) : startTile->rowIndex + (1 * i);
      int colIndex = isLeft ? startTile->colIndex - 1 * i : startTile->colIndex + (1 * i);
...
bool isMoveValidQueen(ChessGame *chessGame, Tile *startTile, Tile *endTile)
{
   bool isRowMove = startTile->rowIndex == endTile->rowIndex;
   bool isColMove = startTile->colIndex == endTile->colIndex;
   bool isDiagMove = !isRowMove & !isColMove;
   return isDiagMove ? isMoveValidBishop(chessGame, startTile, endTile)
                     : isMoveValidRook(chessGame, startTile, endTile);
}

bool isMoveValidKing(ChessGame *chessGame, Tile *startTile, Tile *endTile)
{
   bool isAttemptedCastle = abs(endTile->colIndex - startTile->colIndex) == 2;
   bool isWhite = startTile->pieceColor == 'W';
   if (isAttemptedCastle)
   {
      if (isWhite && (endTile->colIndex != 4 || endTile->rowIndex != 7))
      {
         printf("Invalid castle attempt: your king is not on it's starting square.");
         return false;
      }
      if (!isWhite && (endTile->colIndex != 4 || endTile->rowIndex != 0))
      {
         printf("Invalid castle attempt: your king is not on it's starting square.");
         return false;
      }
      bool isKingSideCastle = endTile->colIndex == 6;
      bool isRookValid = isKingSideCastle ? chessGame->board[startTile->rowIndex][7].piece == 'R'
                                          : chessGame->board[startTile->rowIndex][0].piece == 'R';
      if (!isRookValid)
      {
         printf("Invalid castle attempt: your rook is not on it's starting square.");
         return false;
      }
      if (isKingSideCastle &&
          (chessGame->board[startTile->rowIndex][startTile->colIndex + 1].piece != '-' ||
           chessGame->board[startTile->rowIndex][startTile->colIndex + 2].piece != '-'))
      {
         printf("Invalid castle attempt: there is a piece in the way.");
         return false;
      }
      if (!isKingSideCastle &&
          (chessGame->board[startTile->rowIndex][startTile->colIndex - 1].piece != '-' ||
           chessGame->board[startTile->rowIndex][startTile->colIndex - 2].piece != '-' ||
           chessGame->board[startTile->rowIndex][startTile->colIndex - 3].piece != '-'))
      {
         printf("Invalid castle attempt: there is a piece in the way.");
         return false;
      }
   ...
   for (int i = 0; i < 8; i++)
   {
      ...
      {
         bool isOppositeColor = endTile->pieceColor != startTile->pieceColor;
         if (!isOppositeColor)
         {
...
bool chessIsMoveValid(ChessGame *chessGame, int startRowIndex, int startColumnIndex,
                      int endRowIndex, int endColumnIndex)
{
   ...
   {
   case 'N':
      return isMoveValidKnight(chessGame, startTileP, endTileP);
   case 'P':
      return isMoveValidPawn(chessGame, startTileP, endTileP);
   case 'Q':
      return isMoveValidQueen(chessGame, startTileP, endTileP);
   case 'R':
      return isMoveValidRook(chessGame, startTileP, endTileP);
   case 'B':
      return isMoveValidBishop(chessGame, startTileP, endTileP);
   case 'K':
      return isMoveValidKing(chessGame, startTileP, endTileP);
   }
   return true;
};
...
void chessRegisterMove(ChessGame *chessGame, char *playerMove)
{
   ...
   bool isMoveValid =
       chessIsMoveValid(chessGame, startRowIndex, startColumnIndex, endRowIndex, endColumnIndex);
   if (isMoveValid)
   {
      Tile startTile = chessGame->board[startRowIndex][startColumnIndex];
      Tile endTile = chessGame->board[endRowIndex][endColumnIndex];
